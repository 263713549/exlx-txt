...
def xlsx_to_txt(input_file, output_file=None, sheet_name=None,
               ...
               start_row=1, end_row=None, chunk_size=None, custom_names=None, only_numbers=False, check_duplicates=False, output_dir=None):
    """
    ...
    try:
        ...
        if file_format == 'xlsx':
            success = process_xlsx(input_file, output_file, sheet_name,
                                  delimiter, encoding, header, start_row, end_row, chunk_size, custom_names, only_numbers, check_duplicates, output_dir)
...
def process_xlsx(input_file, output_file, sheet_name, delimiter, encoding, header, start_row, end_row, chunk_size, custom_names, only_numbers, check_duplicates, output_dir=None):
    """处理 XLSX 格式"""
    ...
    return write_chunks(data_rows, header_row, input_file, sheet_name, output_file, delimiter, encoding, chunk_size, custom_names, output_dir)


def process_xls(input_file, output_file, sheet_name, delimiter, encoding, header, start_row, end_row, chunk_size, custom_names, only_numbers, check_duplicates, output_dir=None):
    """处理 XLS 格式"""
...
def process_csv(input_file, output_file, delimiter, encoding, header, start_row, end_row, chunk_size, custom_names, only_numbers, check_duplicates, output_dir=None):
    """处理 CSV 格式"""
...
def write_chunks(data_rows, header_row, input_file, sheet_name, output_file, delimiter, encoding, chunk_size, custom_names, output_dir=None):
    """分块写入文件"""
    ...
    if chunk_size is None or chunk_size >= total_data_rows:
        ...
        write_file(data_rows, header_row, output_path, delimiter, encoding)
        print(f"\n成功！")
        ...
        for chunk_idx in range(num_chunks):
            ...
            output_path = get_output_path(chunk_output_file)
            write_file(chunk_data, header_row if chunk_idx == 0 else None, output_path, delimiter, encoding)
            print(f"  第 {chunk_idx + 1} 个文件: {output_path} ({len(chunk_data)} 行)")
